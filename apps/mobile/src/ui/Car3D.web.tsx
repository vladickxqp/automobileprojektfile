...
// Real WebGL car. Loads a glTF model (CC0 by default, swappable via EXPO_PUBLIC_CAR_MODEL_URL) and
// auto-frames it; if the model can't load (offline, CORS, parse) it falls back to a procedural car
// built from primitives. Drag to orbit, gentle auto-rotate.

const CAR_MODEL_URL =
  process.env.EXPO_PUBLIC_CAR_MODEL_URL ??
...
// ---- Procedural fallback (no asset needed) -------------------------------------------------------
...
function ProceduralCar({ paint, glass }: { paint: string; glass: string }) {
...
export function Car3D({ paint, glass, height = 240, autoRotate = true }: Car3DProps) {
  ...
  const fallback = <ProceduralCar paint={bodyColor} glass={glassColor} />;
